...
public class CatalogueManager {
	...
	public List getArticles() throws Exception {
		...
		try {
			...
			// Query query = session.createQuery("from
			// commerce.catalogue.domaine.modele.Article as m where "+" upper(m.titre) like
			// upper(:paramMotCle)");
			// query.setParameter("paramMotCle", "%illUsiOns%");
			// Query query = session.createQuery("from commerce.domaine.modele.Musique as m
			// where m.prix > 10 and m.prix < 15");
			articles = query.list();
			session.getTransaction().commit();
		} catch (RuntimeException e) {
			if (session.getTransaction() != null)
	...
	public List rechercher(String search) throws Exception {
		Session session = HibernateUtil.getSessionFactory().getCurrentSession();
		try {
			session.beginTransaction();
			Query query = session.createQuery("from commerce.catalogue.domaine.modele.Article as m where upper(m.titre) like upper(:paramMotCle)");
			query.setParameter("paramMotCle", "%" + search + "%");
			articles = query.list();
			session.getTransaction().commit();
		} catch (RuntimeException e) {
			if (session.getTransaction() != null)
